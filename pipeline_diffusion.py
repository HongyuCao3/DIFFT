...
class DiffTipeline(StableDiffusionPipeline):
    def __init__(self, reward_model, scheduler: DDIMScheduler, target = 1, target_guidance = 100): # ,scheduler: DDIMScheduler
    ...
    def __call__(self,
        ...
        for i, t in enumerate(tqdm(timesteps, disable=disable_prog, desc="DDIM Sampling:", leave=False)):
            ...
            noise_pred = diffusion_model.forward(latent_model_input, timestep_tensor, cond = cond)
            # perform guidance
            if do_classifier_free_guidance:
            ...
            if use_reward:
                ...
                noise_pred += (sqrt_1minus_alpha_t * self.target_guidance * computed_gradient).permute(1, 0, 2).contiguous()
