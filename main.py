...
def info_nce_loss(seq_emb, tab_emb, temperature=0.07):
    """计算序列嵌入与表格嵌入之间的对比学习 InfoNCE 损失（双向对齐）

    该函数基于批内匹配的假设：同一批次内索引相同的样本在两个模态中为正样本对。 通过计算 seq→tab 与 tab→seq 两个方向的对比损失，并取其平均作为最终损失。

    Args:
        seq_emb (torch.Tensor): 序列嵌入，形状为 (batch_size, embedding_dim)。
        tab_emb (torch.Tensor): 表格嵌入，形状为 (batch_size, embedding_dim)。
        temperature (float, optional): 温度系数，用于缩放相似度得分，默认值为 0.07。

    Returns: torch.Tensor: 标量张量，表示平均后的 InfoNCE 损失值。

    Raises: RuntimeError: 当输入张量的形状不兼容、设备不一致或矩阵乘法/交叉熵计算失败时，由底层 PyTorch 操作抛出。
    """
...
def pre_training(vae, training_data, validation_data, load_epoch):
    ...
    for epoch in range(load_epoch, args.pre_epochs + 1):
        ...
        for i, batch in enumerate(((training_data))):
            ...
            for u, c in zip(unique.cpu().numpy(), counts.cpu().numpy()):
                if u in chunk_size_gt_distribution:
                    ...
                    chunk_size_gt_distribution[u] = c
            logits, mean, logvar, evaluation, seq_emb, tab_emb, chunk_size = vae(
                seq, tab, chunk
            )

            # Prepare target sequence for reconstruction loss
            merged_target_seq_index = [
                sub_target_seq.squeeze(0) for sub_target_seq in chunk
            ]
            ...
            kl_loss = (
                -0.5 * torch.sum(1 + logvar - mean.pow(2) - logvar.exp()) / seq.size(0)
            )
        ...
        if epoch % 1 == 0:
            info(
                f"Pre-training Epoch {epoch}: Reconstruction Loss = {total_reconstruction_loss / len(training_data)}, Performance Loss = {total_performance_loss / len(training_data)}, Alignment Loss = {total_alignment_loss / len(training_data)}, Chunk Loss = {total_chunk_loss/len(training_data)}, KL Loss = {total_kl_loss / len(training_data)}, Accuracy = {correct / total_chunk}"
            )
...
def infer(vae, data, device, df):
    ...
    Args:
        vae: 具有 eval 与 generate_test 方法的模型（通常为 torch.nn.Module），用于根据输入序列生成操作序列。
        data: 可迭代的批数据，每个批次应包含键 "seqs"、"tabs"、"performances"、"chunk_seqs"，其中至少会使用 "seqs"。
        device: 推理所用的设备（如 "cpu"、"cuda" 或 torch.device 实例）。
        df: 包含特征和标签的数据表，要求最后一列为标签，其余列为特征。

    Returns: int: 固定返回 0。本函数主要通过日志输出展示基线与更新后的评估指标。

    Raises:
        RuntimeError: 模型前向或张量设备/形状不匹配等导致的运行时错误。
        ValueError: 下游评估或数据拼接过程中出现的无效输入等问题。
        Exception: 其他未预见的错误。单条序列的解析异常会被内部忽略，不会中断整体流程。"""
...
def main():
    ...
    info(
        f"Max length in training data is:{max_length}, vocab size is: {dataset.vocab_size}"
    )
    ...
    if args.load_epoch > 0 and os.path.exists(
        os.path.join(model_path, f"vae_{args.load_epoch}{args.keyword}.pt")
    ):
        vae.load_state_dict(
            torch.load(
                os.path.join(model_path, f"vae_{args.load_epoch}{args.keyword}.pt")
